<?php
 include ("config.php");
  ...
  $result3 = mysql_query("SELECT * FROM advert WHERE id_users='$user_id'",$db); 
...
<?php
  $result = mysql_query("SELECT * FROM user WHERE id_user='$user_id'",$db); 
  $myrow = mysql_fetch_array($result);
?>
                                    <?php  echo '<img class="img-thumbnail" style="width: 150px; height: 150px" src="data:image/jpeg;base64,' . base64_encode( $myrow['photo_user'] ) . '" />'?> 
                                </div>
                                <div class="userData ml-3">
                                    <h2 class="d-block" style="font-size: 1.5rem; font-weight: bold"><a href="javascript:void(0);"><?php echo $myrow['fio_user']; ?></a></h2>
                                    <h6 class="d-block"><a href="javascript:void(0)"><?php echo $num; ?></a> Объявления было создано пользователем</h6>
<?php 
if($myrow['block_user']=='0')
{
?>  
                                                                                    <a href="blocked.php?id=<?php echo $myrow['id_user'];?>" class="btn btn-danger">Заблокировать пользователя </a>
                                                                                    <?php
    if($myrow['admin_user']==0)
    {
?>
                                                                                    <a href="admined.php?id=<?php echo $myrow['id_user'];?>" class="btn btn-primary">Назначить администратором</a>
...
<?php
    ...
    {
?>
                                                                                    <a href="#" class="btn btn-success">Администратор</a>
<?php        
}}
else
{   
?>
                                                                                    ...
                                                                                    <a href="unblocked.php?id=<?php echo $myrow['id_user'];?>" class="btn btn-primary">Разблокировать пользователя </a>
<?php
}
?>                           
                                </div>
                                    ...
                                    <div class="tab-pane fade" id="ads" role="tabpanel" aria-labelledby="ads-tab">
                                         <div class="container-fluid">
                                                <div class="col-lg-12">
                                                    <div class="row">
<?php
$resultUs = mysql_query("SELECT * FROM    user WHERE id_user='$user_id'",$db); 
while($myrowUs = mysql_fetch_array($resultUs))
{
  $resultAd = mysql_query("SELECT * FROM advert WHERE id_users='$user_id'",$db);
